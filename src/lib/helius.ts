...
// Helius API configuration (server-only)
// IMPORTANT: Do NOT expose this key client-side. Set HELIUS_API_KEY in Vercel/hosting env
const HELIUS_API_KEY = process.env.HELIUS_API_KEY || '';
const HELIUS_RPC_URL = `https://devnet.helius-rpc.com/?api-key=${HELIUS_API_KEY}`;

if (!HELIUS_API_KEY) {
  console.warn(
    '⚠️ HELIUS_API_KEY not set. Set HELIUS_API_KEY in your server environment (do NOT use NEXT_PUBLIC_HELIUS_API_KEY for production deployments).'
  );
}
...
export async function getAssetsByOwner(
  ...
  try {
    // If no API key is configured, avoid throwing in client code — return empty list
    // so the UI can render without crashing. callDASApi is strict and will throw
    // if invoked without a key, so short-circuit here.
    if (!HELIUS_API_KEY) {
      console.warn('Skipping Helius DAS API call: no API key configured. Returning empty cNFT list.');
      return [];
    }
